...
public static class PurviewAutomation
{
    ...
    public static async Task RunAsync([EventGridTrigger] EventGridEvent eventGridEvent, ILogger log)
    {
        ...
        var setupManagedPrivateEndpoints = Convert.ToBoolean(GetEnvironmentVariable(name: "SetupManagedPrivateEndpoints"));

        switch (eventDetails.Operation)
        {
            case "write":
                log.LogInformation($"Write opreation detected");
                await AddDataSourceAsync(eventDetails: eventDetails, purviewAutomationClient: purviewAutomationClient, setupManagedPrivateEndpoints: setupManagedPrivateEndpoints, setupScan: setupScan, triggerScan: triggerScan, setupLineage: setupLineage, functionPrincipalId: functionPrincipalId, logger: log);
                break;
            case "delete":
    ...
    /// <returns></returns>
    private static async Task AddDataSourceAsync(EventDetails eventDetails, PurviewAutomationClient purviewAutomationClient, bool setupManagedPrivateEndpoints, bool setupScan, bool triggerScan, bool setupLineage, string functionPrincipalId, ILogger logger)
    {
        if (eventDetails.Action == "microsoft.storage/storageaccounts/write")
        {
            ...
            await storageOnboardingClient.OnboardDataSourceAsync(setupManagedPrivateEndpoints: setupManagedPrivateEndpoints, setupScan: setupScan, triggerScan: triggerScan);
        }
        else if (eventDetails.Action == "microsoft.synapse/workspaces/write")
        {
            ...
            await synapseOnboardingClient.OnboardDataSourceAsync(setupManagedPrivateEndpoints: setupManagedPrivateEndpoints, setupScan: setupScan, triggerScan: triggerScan);
        ...
        else if (eventDetails.Action == "microsoft.kusto/clusters/write")
        {
            ...
            await kustoOnboardingClient.OnboardDataSourceAsync(setupManagedPrivateEndpoints: setupManagedPrivateEndpoints, setupScan: setupScan, triggerScan: triggerScan);
        }
        else if (eventDetails.Action == "microsoft.documentdb/databaseaccounts/write")
        {
            ...
            await cosmosOnboardingClient.OnboardDataSourceAsync(setupManagedPrivateEndpoints: setupManagedPrivateEndpoints, setupScan: setupScan, triggerScan: triggerScan);
        }
        else if (eventDetails.Action == "microsoft.sql/servers/write")
        {
            ...
            await sqlServerOnboardingClient.OnboardDataSourceAsync(setupManagedPrivateEndpoints: setupManagedPrivateEndpoints, setupScan: setupScan, triggerScan: triggerScan);
        }
        else if (eventDetails.Action == "microsoft.sql/servers/databases/write")
        {
            ...
            await sqlDatabaseOnboardingClient.OnboardDataSourceAsync(setupManagedPrivateEndpoints: setupManagedPrivateEndpoints, setupScan: setupScan, triggerScan: triggerScan);
        }
        else
        {
